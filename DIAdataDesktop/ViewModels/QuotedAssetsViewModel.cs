...
namespace DIAdataDesktop.ViewModels
{
    public partial class QuotedAssetsViewModel : ObservableObject
    {
        ...
        // NEW: enrich cancellation (no cache)
        ...
        // tune: how many parallel quotation calls
        ...
        private static string KeyOf(DiaQuotedAsset a)
            => $"{(a.Asset.Blockchain ?? "").Trim()}|{(a.Asset.Address ?? "").Trim()}".ToLowerInvariant();

        private static string KeyOf(DiaQuotedAssetRow r)
            => $"{(r.Blockchain ?? "").Trim()}|{(r.Address ?? "").Trim()}".ToLowerInvariant();
        ...
        private void ApplyFilterCore()
        {
            ...
            if (!string.IsNullOrWhiteSpace(bc) &&
                !string.Equals(bc, "(All)", StringComparison.OrdinalIgnoreCase))
            {
        ...
        public async Task LoadQuotedAssetsAsync()
        {
            ...
            try
            {
                ...
                // IMPORTANT: fill quotations (price/time) for visible page
                await EnrichVisiblePageAsync();
            }
            catch (OperationCanceledException)
            {
        ...
        // NEW: fills Price/Time/etc. by loading quotations for currently visible rows
        private async Task EnrichVisiblePageAsync()
        {
            _enrichCts?.Cancel();
            _enrichCts = new CancellationTokenSource();
            var ct = _enrichCts.Token;

            List<DiaQuotedAssetRow> rows;
            try
            {
                rows = await _ui.InvokeAsync(() => PagedRows.ToList());
            }
            catch
            {
                return;
            ...
            var tasks = rows.Select(async row =>
            {
                ...
                try
                {
                    // if quotation already exists and looks valid, skip
                    if (row.Quotation != null && row.Price > 0) return;
                    ...
                    await _ui.InvokeAsync(() =>
                    {
                        row.UpdateQuotation(q);
                    }, DispatcherPriority.Background);
                }
                catch (OperationCanceledException) { }
                catch
                {
                    // swallow per-row failures (don’t kill whole page)
                }
                finally
                {
                    sem.Release();
